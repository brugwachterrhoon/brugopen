...
function currentValueInMetresPerSecond(value, unit) {
  const normalized = String(unit).toLowerCase().replace(/\s+/g, "");
  if (normalized === "cm/s" || normalized === "cms") return value / 100;
  if (normalized === "mm/s" || normalized === "mms") return value / 1000;
  if (normalized === "km/h" || normalized === "kmh") return value / 3.6;
  if (["kn", "knot", "knopen"].includes(normalized)) return value * 0.514444;
  return value;
}
...
function parseWfsSeriesCsv(text, expectedQuantity) {
  ...
  for (const values of rows.slice(1)) {
    ...
    const defaultUnit =
      ["WINDSHD", "STROOMSHD"].includes(expectedQuantity) ? "m/s" :
      ["WINDRTG", "STROOMRTG"].includes(expectedQuantity) ? "graad" : "cm";
...
function combineCurrentSeries(speedRows, directionRows) {
  const directionByCode = new Map();
  for (const row of directionRows) {
    const current = directionByCode.get(row.locationCode);
    if (!current || timestamp(row.measuredAt) > timestamp(current.measuredAt)) {
  ...
  return speedRows.flatMap((speed) => {
    const valueMps = currentValueInMetresPerSecond(speed.value, speed.unit);
    if (!Number.isFinite(valueMps) || valueMps < 0 || valueMps >= 15) return [];
    const direction = directionByCode.get(speed.locationCode);
    return [{
      locationCode: speed.locationCode,
      locationName: speed.locationName,
...
async function downloadCurrentStations() {
  const [speedRows, directionRows] = await Promise.all([
    fetchWfsSeries("STROOMSHD"),
    fetchWfsSeries("STROOMRTG").catch(() => [])
  ]);
  return combineCurrentSeries(speedRows, directionRows);
}
...
function unavailableCurrent(message = "Geen actuele stroommeting beschikbaar") {
  return {
    currentSpeedMps: null,
    currentDirectionDegrees: null,
    currentMeasuredAt: null,
    currentLocationCode: null,
    currentLocationName: "RWS stroommeetpunt",
...
function mergeCurrentData(data, stations, previousData = null) {
  const previousById = new Map((previousData?.bridges ?? []).map((bridge) => [bridge.id, bridge]));
  return {
    ...data,
    currentSource: "Rijkswaterstaat Waterinfo",
    currentSourceUrl: WATER_SOURCE_URL,
    bridges: data.bridges.map((bridge) => {
      const fresh = currentForBridge(bridge, stations);
      if (fresh.currentSpeedMps !== null) return { ...bridge, ...fresh };
      const previous = previousById.get(bridge.id);
      if (previous?.currentSpeedMps !== null && previous?.currentSpeedMps !== undefined) {
        return {
          ...bridge,
          currentSpeedMps: previous.currentSpeedMps,
          currentDirectionDegrees: previous.currentDirectionDegrees,
          currentMeasuredAt: previous.currentMeasuredAt,
          currentLocationCode: previous.currentLocationCode,
          currentLocationName: previous.currentLocationName,
          currentDistanceKm: previous.currentDistanceKm,
          currentStatus: "stale",
          currentMessage: "Laatste succesvolle RWS-stroommeting",
          currentSourceUrl: WATER_SOURCE_URL
        };
      }
      return { ...bridge, ...fresh };
...
async function refreshData({ force = false } = {}) {
  ...
  state.refreshing = (async () => {
    ...
    try {
      const currentStations = await downloadCurrentStations();
      dashboardData = mergeCurrentData(dashboardData, currentStations, previousData);
    } catch (error) {
      errors.push(`RWS stroming: ${error instanceof Error ? error.message : String(error)}`);
      dashboardData = mergeCurrentData(dashboardData, [], previousData);
    }
...
function currentFlow(b){
  if(typeof b.currentSpeedMps!=='number')return {value:'—',unit:'m/s',direction:'',degrees:0,detail:b.currentMessage||'Geen actuele stroommeting'};
  const direction=compassDirection(b.currentDirectionDegrees),bits=[];
  if(direction)bits.push('naar '+direction);
  if(typeof b.currentDistanceKm==='number')bits.push(b.currentDistanceKm.toLocaleString('nl-NL',{maximumFractionDigits:1})+' km');
  if(b.currentMeasuredAt)bits.push(timeFmt.format(new Date(b.currentMeasuredAt)));
  return {value:b.currentSpeedMps.toLocaleString('nl-NL',{minimumFractionDigits:1,maximumFractionDigits:2}),unit:'m/s',direction,degrees:typeof b.currentDirectionDegrees==='number'?b.currentDirectionDegrees:0,detail:bits.join(' · ')||'Richting niet beschikbaar'};
}
function live(b){
...
function render(data){
  ...
  for(const b of data.bridges){
    const o=opportunity(b),f=followingOpportunity(b),w=water(b),v=wind(b),c=currentFlow(b),l=live(b);
